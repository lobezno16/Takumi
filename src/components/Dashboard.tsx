...
import { MOCK_TREND_DATA, DISTRICTS } from "../mockData";
...
export default function Dashboard() {
  const { deliveries, vehicles, setCurrentPage, setSelectedDeliveryId, setActiveVehicleId } = useDeliveryStore();
  ...
  const confirmedDeliveries = deliveries.filter((d) => d.status === "confirmed").length;
  const averagePHome = parseFloat((deliveries.reduce((sum, d) => sum + d.pHome, 0) / totalDeliveriesCount).toFixed(2));
  
  // Calculate dynamic metrics based on live state
  // Standard redelivery rate drops as confirmed ratio increases
  const liveRedeliveryRate = parseFloat((32.6 - (confirmedDeliveries / totalDeliveriesCount) * 34).toFixed(1));
  const liveHoursSaved = parseFloat((confirmedDeliveries * 0.22).toFixed(1));
  const liveCo2Saved = parseFloat((confirmedDeliveries * 0.51).toFixed(1));

  // Simulated KPI numbers with live calculations matching mock ratios
  const kpis = [
    {
      id: "redelivery",
