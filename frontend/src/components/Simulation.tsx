...
export default function Simulation() {
  ...
  // Real deltas from the backend run
  const improvement = results ? results.improvementPct : 0;
  const hoursDeltaPct =
    results && results.metrics.baseline.driverHours > 0
      ? (1 - results.metrics.takumi.driverHours / results.metrics.baseline.driverHours) * 100
      : 0;
  const bench = results?.benchmark ?? {
    ortools: { timeMs: 0, gap: 0 },
    pyvrp: { timeMs: 0, gap: 0 },
  };
  ...
  return (
    <div id="simulation-page" className="p-6 space-y-6 select-none bg-ink h-auto min-h-[calc(100vh-64px)] text-xs">
        ...
        <div id="simulation-config-panel" className="bg-paper text-ink rounded-lg p-5 shadow-md flex flex-col justify-between">
          <div className="space-y-4">
            ...
            <div className="space-y-1.5 pt-2">
              ...
              />
              <span className="text-[9px] text-muted block">配送件数：20件から200件をバックエンドで実解求（OR-Tools）</span>
            </div>
      ...
      {!isSimulating && simError && (
        <div id="simulation-error-banner" className="bg-accent/10 border border-accent/30 rounded-lg p-4 text-center max-w-xl mx-auto">
          <span className="text-xs text-accent font-semibold">Simulation failed: {simError}</span>
        </div>
      )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="bg-paper text-ink p-4 rounded-lg shadow-md flex flex-col justify-between h-36">
              ...
              <div className="text-[10px] text-muted border-t border-neutral-200 pt-1.5 flex justify-between items-center">
                ...
                <span className="font-bold text-signal font-mono">↓ {improvement.toFixed(1)}% vs baseline</span>
              </div>
            </div>
            ...
            <div className="bg-paper text-ink p-4 rounded-lg shadow-md flex flex-col justify-between h-36">
              ...
              <div className="text-[10px] text-muted border-t border-neutral-200 pt-1.5 flex justify-between items-center">
                ...
                <span className="font-bold text-accent font-mono">↓ {hoursDeltaPct.toFixed(0)}% hours cut</span>
              </div>
            </div>
            ...
            <div className="bg-paper text-ink p-4 rounded-lg shadow-md flex flex-col justify-between h-36">
              ...
              <div className="text-[10px] text-muted border-t border-neutral-200 pt-1.5 flex justify-between items-center">
                ...
                <span className="font-bold text-signal font-mono">↓ {hoursDeltaPct.toFixed(0)}% carbon saved</span>
              </div>
            </div>
                ...
                <div className="p-3 bg-[#0D1117] rounded-md border border-[#22272E] flex justify-between items-center">
                  ...
                  <div className="text-right">
                    <span className="font-mono text-xs font-semibold text-paper block">
                      {bench.ortools.timeMs}ms
                    </span>
                    <span className="text-[8px] text-muted block">reference</span>
                  </div>
                </div>
                ...
                <div className="p-3 bg-accent/10 rounded-md border border-accent/25 flex justify-between items-center">
                  ...
                  <div className="text-right">
                    <span className="font-mono text-xs font-bold text-signal block">
                      {bench.pyvrp.timeMs}ms
                    </span>
                    <span className="text-[8px] text-muted block font-semibold">gap: {bench.pyvrp.gap.toFixed(1)}%</span>
                  </div>
                </div>
              </div>

              <div className="text-[9px] text-muted italic leading-relaxed pt-3 border-t border-[#22272E]/50 mt-4 leading-normal">
                ⭐ {bench.pyvrp.timeMs > 0 ? (
                  <>
                    Same seeded instance, both solvers: <strong>PyVRP</strong> finished in{" "}
                    <strong>{bench.pyvrp.timeMs}ms</strong> vs OR-Tools {bench.ortools.timeMs}ms, with a
                    total-route-time gap of {bench.pyvrp.gap.toFixed(1)}% (negative = OR-Tools shorter).
                    Production routing keeps OR-Tools for its richer prize-collecting objective.
                  </>
                ) : (
                  <>Benchmark data unavailable for this run — the comparison endpoint may have timed out.</>
                )}
              </div>
            </div>
