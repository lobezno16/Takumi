...
export default function RouteMap() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const drawRouteLines = () => {
        vehicles.forEach((v) => {
          ...
          const baselineCoordinates = v.baselineCoordinates;
    ...
    // viewState is intentionally omitted: the map instance is the source of
    // truth for camera state and writes back via the "move" handler above.
  ...
  // Deliver the typed text to the real constrained agent loop (recipient side).
  const handleSidebarSendMessage = () => {
    if (!sidebarChatInput.trim() || !currentSelectedDelivery) return;
    const msgText = sidebarChatInput;
    setSidebarChatInput("");
    void sendRecipientMessage(currentSelectedDelivery.id, msgText);
  };
            ...
            <div className="flex items-center justify-between">
              <div className="flex flex-col">
                ...
                <span className="text-[10px] text-muted">
                  車両別ルートトグル・運行状況 ({vehicles.length}台)
                </span>
              </div>
            ...
            <div id="tactical-vector-stage" className="w-full h-full bg-[#090C10] relative flex items-center justify-center p-6 select-none overflow-hidden">
              ...
              >
                ...
                {vehicles.map((v) => {
                  ...
                  const baselinePoints = v.baselineCoordinates.map(([clng, clat]) => {
                    const p = projectCoord(clng, clat, 800, 600);
                    return `${p.x},${p.y}`;
                  }).join(" ");
                ...
                {filteredDeliveries.map((del) => {
                  ...
                  return (
                    <g 
                      ...
                      onMouseEnter={() => setHoveredStop({ x: p.x, y: p.y, delivery: del })}
