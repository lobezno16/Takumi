...
export default function DeliveryDetail() {
  const { deliveries, selectedDeliveryId, setSelectedDeliveryId } = useDeliveryStore();
  const { chatThreads, sendRecipientMessage } = useAgentStore();
  ...
  const delivery = useMemo(() => {
    ...
    if (!del && deliveries.length > 0) {
      del = deliveries.find((d) => d.orderId) || deliveries[0];
    }
    return del;
  ...
  // Deliver the typed text to the real constrained agent loop. The input
  // plays the recipient side of the conversation (like a LINE reply); the
  // backend extracts at most a SlotCode and replies accordingly.
  const handleSendMessage = () => {
    if (!chatInputValue.trim() || !delivery) return;
    const typedText = chatInputValue;
    setChatInputValue("");
    void sendRecipientMessage(delivery.id, typedText);
  };
          ...
          <div id="agent-thread-panel" className="bg-paper text-ink rounded-lg p-5 shadow-md flex flex-col h-[284px]">
            <div className="flex justify-between items-center pb-2.5 border-b border-neutral-200 shrink-0">
              ...
              <div className="flex items-center gap-1 text-[10px] text-muted font-mono bg-neutral-200 py-0.5 px-2 rounded">
                <span className={`h-1.5 w-1.5 rounded-full ${delivery.orderId ? "bg-signal" : "bg-warn"}`} />
                <span>{delivery.orderId ? "Agent Live" : "Not Enrolled"}</span>
              </div>
            </div>
            ...
            <div className="flex gap-2 pt-2 border-t border-neutral-200 shrink-0">
              <input
                id="chat-send-input"
                type="text"
                placeholder={delivery.orderId ? "Reply as recipient: e.g. 'I am home after 6pm today'" : "Stop not enrolled in outreach (low absence risk)"}
