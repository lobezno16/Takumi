...
const FEATURE_LABELS: Record<string, string> = {
  historical_hit_rate: "Historical Hit Rate (stop x slot)",
  overall_hit_rate: "Overall Recipient Attendance Index",
  slot_code_encoded: "Delivery Time Slot Encoding",
  floor: "Building Floor Level",
  address_type_encoded: "Housing Type (apartment/house)",
  day_of_week: "Day of Week",
  is_weekday: "Weekday vs Weekend Signature",
  historical_count: "Observed Attempt Count (stop x slot)",
};

export default function MLHealth() {
  const { metrics, isTraining, mlError, train } = useMlStore();
  const [driftOverride, setDriftOverride] = useState<number | null>(null);

  // Train once on first visit; metrics persist for the session.
  useEffect(() => {
    if (!metrics && !isTraining && !mlError) void train();
  }, [metrics, isTraining, mlError, train]);

  // Real calibration curve from the held-out test split.
  const calibrationData = (metrics?.calibration_curve ?? []).map((b) => ({
    name: `${Math.round(b.bin_start * 100)}-${Math.round(b.bin_end * 100)}%`,
    perfect: parseFloat((b.mean_predicted * 100).toFixed(1)),
    model: parseFloat((b.observed_rate * 100).toFixed(1)),
  }));
  ...
  // Live drift proxy = mean |observed - predicted| across calibration bins.
  const computedDrift =
    metrics && metrics.calibration_curve.length > 0
      ? metrics.calibration_curve.reduce(
          (s, b) => s + Math.abs(b.observed_rate - b.mean_predicted),
          0,
        ) / metrics.calibration_curve.length
      : 0.02;
  const driftError = driftOverride ?? parseFloat(computedDrift.toFixed(3));
  const setDriftError = (v: number) => setDriftOverride(v);
  ...
  const models = metrics
    ? [
        {
          version: `lgbm-${metrics.calibration_method}-calibrated`,
          trainedAt: `${metrics.train_size} train / ${metrics.test_size} test rows`,
  ...
  if (!metrics) {
    return (
      <div className="p-6 min-h-[calc(100vh-64px)] bg-ink flex flex-col items-center justify-center gap-3 select-none">
        {mlError ? (
          <>
            <span className="text-sm font-bold text-accent">Model training failed</span>
            <span className="text-[11px] text-muted max-w-md text-center">{mlError}</span>
            <button
              onClick={() => void train()}
              className="mt-2 px-4 py-2 bg-accent text-paper text-xs font-semibold rounded hover:bg-amber-bright transition-colors"
            >
              Retry Training
            </button>
          </>
          ...
          <>
            ...
            <span className="text-xs font-mono tracking-wider text-paper animate-pulse">
              Training calibrated LightGBM on synthetic history…
            </span>
            <span className="text-[10px] text-muted">
              Platt scaling · 5-fold CV · Brier + log-loss quality gates
            </span>
          </>
        )}
      </div>
          ...
          <div className="relative z-10 grid grid-cols-1 lg:grid-cols-5 gap-6 items-center">
            ...
            <div className="lg:col-span-2 bg-[#161B22]/85 backdrop-blur-md border border-[#22272E] p-4 rounded-lg flex flex-col justify-between space-y-3">
              ...
              <button
                ...
                onClick={() => void train()}
                disabled={isTraining}
                className={`w-full py-2 rounded text-[10px] font-semibold tracking-wide flex items-center justify-center gap-1.5 transition-all ${
                  isTraining
              ...
              >
                ...
                {isTraining ? "Training LightGBM…" : "Retrain Model (synthetic 12w)"}
              </button>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-stretch">
        ...
        <div id="ml-calibration-curve-card" className="bg-paper text-ink rounded-lg p-5 shadow-md flex flex-col justify-between">
          <div className="flex flex-col mb-4">
            <div className="flex justify-between items-start">
              ...
              <span className="text-[10px] font-semibold text-data uppercase font-mono bg-data/10 px-2 py-0.5 rounded">
                Brier: {metrics.brier_score.toFixed(3)}
              </span>
            </div>
          ...
          <div className="h-64 w-full">
            <ResponsiveContainer width="100%" height="100%">
              ...
              >
                ...
                <Line
                  name="Mean predicted p_home（予測平均）"
                  type="monotone"
                  dataKey="perfect"
                  stroke="#6B7280"
                  strokeDasharray="5 5"
                ...
                <Area
                  name="Observed attendance（実測在宅率）"
                  type="monotone"
                  dataKey="model"
                  stroke="#4A9EFF"
                  fill="#4A9EFF"
          ...
          <div className="text-[9.5px] text-muted italic leading-relaxed pt-2.5 border-t border-neutral-200 mt-2 font-sans">
            Held-out split calibration: Brier {metrics.brier_score.toFixed(3)}, log-loss{" "}
            {metrics.log_loss.toFixed(3)}, accuracy {(metrics.accuracy * 100).toFixed(1)}% — a predicted
            80% p_home corresponds to ≈80% observed attendance.
          </div>
        </div>
        ...
        <div id="ml-feature-importance-card" className="bg-paper text-ink rounded-lg p-5 shadow-md flex flex-col justify-between">
          ...
          <div className="text-[9.5px] text-muted italic leading-relaxed pt-2.5 border-t border-neutral-200 mt-2 font-sans">
            <strong>{featureData[0]?.name ?? "Historical hit rate"}</strong> carries the highest
            LightGBM split weight ({((featureData[0]?.importance ?? 0) * 100).toFixed(0)}%), confirming
            the recall loop: more delivery history → better windows → fewer redeliveries.
          </div>
        </div>
          ...
          <table className="w-full text-left text-xs">
            <thead>
              <tr className="bg-[#0D1117] border-b border-[#22272E] text-muted font-mono uppercase text-[9px] font-bold">
                ...
                <th className="p-3">Training Split</th>
